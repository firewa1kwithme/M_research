clear;
% грубые оценки м-к, самые первые
...
sle_t_ = zeros(Nx-2,Ny-2);
sle_bc = zeros(Nx-2,Ny-2);
sle_bc_ = zeros(Nx-2,Ny-2);
sle_pw1 = zeros(Nx-2,Ny-2);
sle_pw1_ = zeros(Nx-2,Ny-2);
sle_pw2 = zeros(Nx-2,Ny-2);
sle_pw2_ = zeros(Nx-2,Ny-2);
...
for i = 1:N
 ...
 % реализации систем линейных уравнений с различными входными данными
    ...
    sle_t_ = sle(T_x, T_y, bc_1, pw1_1, pw2_1); % f(y',z) with respect to t
    sle_bc = sle(T_x, T_y, bc_1, pw1, pw2); % f(y,z') with respect to bc
    sle_bc_ = sle(Tx_1, Ty_1, bc, pw1_1, pw2_1); % f(y',z) with respect to bc
    sle_pw1 = sle(T_x, T_y, bc, pw1_1, pw2); % f(y,z') with respect to pw1
    sle_pw1_ = sle(Tx_1, Ty_1, bc_1, pw1, pw2_1); % f(y',z) with respect to pw1
    sle_pw2 = sle(T_x, T_y, bc, pw1, pw2_1); % f(y,z') with respect to pw2
    sle_pw2_ = sle(Tx_1, Ty_1, bc_1, pw1_1, pw2); % f(y',z) with respect to pw2
    
    g = Nx-2;
    RES(:,1:g,i)=sle0;
    RES(:,g+1:2*g,i)=sle_t; % f(y,z')
    RES(:,2*g+1:3*g,i)=sle_t_;
    RES(:,3*g+1:4*g,i)=sle_bc; % f(y,z')
    RES(:,4*g+1:5*g,i)=sle_bc_;
    RES(:,5*g+1:6*g,i)=sle_pw1; % f(y,z')
    RES(:,6*g+1:7*g,i)=sle_pw1_;
    RES(:,7*g+1:8*g,i)=sle_pw2; % f(y,z')
    RES(:,8*g+1:9*g,i)=sle_pw2_;
...
for iNA = 1:N
    ...
    DyT = DyT+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,g+1:2*g,iNA)-f_0(:,g+1:2*g));
    DzT = DzT+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,2*g+1:3*g,iNA)-f_0(:,2*g+1:3*g));
    
    DyBC = DyBC+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,3*g+1:4*g,iNA)-f_0(:,3*g+1:4*g));
    DzBC = DzBC+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,4*g+1:5*g,iNA)-f_0(:,4*g+1:5*g));
    
    DyPW1 = DyPW1+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,5*g+1:6*g,iNA)-f_0(:,5*g+1:6*g));
    DzPW1 = DzPW1+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,6*g+1:7*g,iNA)-f_0(:,6*g+1:7*g));
    
    DyPW2 = DyPW2+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,7*g+1:8*g,iNA)-f_0(:,7*g+1:8*g));
    DzPW2 = DzPW2+(RES(:,1:g,iNA)-f_0(:,1:g)).*(RES(:,8*g+1:9*g,iNA)-f_0(:,8*g+1:9*g));
...
fintime=toc;
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
path = 'C:\Users\your_mom\Documents\MATLAB\MR\results\final\crude estimates\01\';
path2 = sprintf('%s%s_%s_%s S', path, num2str(Nx), num2str(Ny), num2str(N));
...
c=colorbar;
c.Limits=[0 1];
title('S_y for pressure in w_1');
saveas(h3, [path2 '\SyPW1.png'], 'png');
h7 = imagesc(SyPW1tot);
c=colorbar;
c.Limits=[0 1];
title('total S_y for pressure in w_1');
